<?php
 ...
 $primary_docs = array(
  ...
  'abz' => array(xl('Advanced Billing')  , 0, '../mymodules/index.php?billing_module=1'),
...
$(document).ready(function(){
  if(3 == <?php echo $GLOBALS['concurrent_layout'] ?>){
    ...
    $("#navigation-slide > li  > a#abz0").prepend('<img src="../../images/advbilling.png" class="nav-menu-img" />');
...
?>
  ...
  <li><a class="collapsed" id="misimg" ><span><?php xl('Miscellaneous','e') ?></span></a>
    <ul>
      <?php if (!$GLOBALS['external_module_path'] || !$GLOBALS['external_module_username'] || !$GLOBALS['external_module_password']) genTreeLink('RBot','aun',xl('Authorizations')); ?>
...
<?php } else { // not athletic team ?>
  ...
  <?php if($GLOBALS['portal_offsite_enable'] && $GLOBALS['portal_offsite_address'] && acl_check('patientportal','portal'))  genTreeLink('RTop','app',xl('Portal Activity'));
	if ($GLOBALS['external_module_path'] && $GLOBALS['external_module_username'] && $GLOBALS['external_module_password']) genTreeLink('RTop','abz',xl('Advanced Billing')); ?>
...
<?php } // end if gbl_nav_visit_forms ?>
  ...
  <li><a class="collapsed" id="feeimg" ><span><?php xl('Fees','e') ?></span></a>
    <ul>
      <?php
        if (!$GLOBALS['external_module_path'] || !$GLOBALS['external_module_username'] || !$GLOBALS['external_module_password']){
      ?>
      <?php genMiscLink('RBot','cod','2',xl('Fee Sheet'),'patient_file/encounter/load_form.php?formname=fee_sheet'); ?>
